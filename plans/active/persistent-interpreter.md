...
- **Launcher interface: `run.py --new-session - [script arguments...]`, then `run.py --session ID - [script arguments...]`.** Session cells read code from stdin, exactly like `run.py -`. A file argument in session mode is rejected: running a file as a cell would silently change `__file__`, `sys.path[0]` and relative imports, which is the second dialect the explicit-imports decision rejects. `--session ID --reset` discards bindings without replacing the interpreter, `--timeout SECONDS` sets the per-cell deadline (default 300 s), `--ttl SECONDS` sets the idle timeout, and `--kill-session ID` and `--list-sessions` control and discover live sessions.
...
- **Socket location and record.** One socket per session in `$XDG_RUNTIME_DIR/surf-agent/`, with the state directory as the fallback when `XDG_RUNTIME_DIR` is unset. Socket mode is 0600 and the directory 0700. The session id is the socket file name, and that file is the whole session record: no registry, no log, no lock file.
- **Session discovery.** `--list-sessions` scans that directory, asks each socket for its interpreter's pid, cell count, idle time, timeout and working directory, and sweeps sockets that have no listener. Because ids are unique by construction, two sessions cannot share an interpreter, and no owner inference is needed to keep them apart.
