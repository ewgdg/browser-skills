...
- **The worker is this interpreter, from the launcher's own uv project.** `run.py` keeps a small project under the cache directory (`SURF_AGENT_ENV_DIR` to move it) whose `pyproject.toml` pins the requirement, lets `uv sync` own the `.venv` and its lock file, and executes that interpreter directly; the worker therefore starts from a directory that outlives every launcher call. This replaces the earlier `SURF_SESSION_WORKER_COMMAND` handoff, which existed only because `uv run --with` builds an environment under a temporary path and deletes it with the process that made it, forcing a uv parent to stay alive for the whole session: measured at 39 MiB of private memory per session on top of the worker's 35 MiB, where this environment costs nothing per session. `uv tool install` is not an option: it requires console scripts, and this package deliberately has none, so uv removes the tool it just built. Direct `python -m surf_agent.session` use keeps spawning with `sys.executable`.
