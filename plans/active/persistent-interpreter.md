...
- **The worker is this interpreter, from the launcher's own environment.** `run.py` installs the pinned requirement once into an environment keyed by it (under the cache directory, `SURF_AGENT_ENV_DIR` to move it), prunes environments no process is using, and executes that environment's Python directly; the worker therefore starts from a directory that outlives every launcher call. This replaces the earlier `SURF_SESSION_WORKER_COMMAND` handoff, which existed only because `uv run --with` builds an environment under a temporary path and deletes it with the process that made it, forcing a uv parent to stay alive for the whole session: measured at 39 MiB of private memory per session on top of the worker's 35 MiB, where this environment costs nothing per session. Direct `python -m surf_agent.session` use keeps spawning with `sys.executable`.
