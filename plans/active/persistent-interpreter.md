...
- **The worker is started by the launcher's own dependency command.** Under `uv run --with`, `sys.executable` points into a temporary environment that uv deletes when its caller exits, so a detached worker could not start the Patchright bridge from a later cell. `run.py` passes its resolved command through `SURF_SESSION_WORKER_COMMAND`; the worker re-enters uv's resolution, which keeps that environment alive for the worker's lifetime. Warm `uv run` adds roughly 30 ms to spawn-to-ready. Direct `python -m surf_agent.session` use keeps spawning with `sys.executable`. One environment per call rather than one per installation is deliberate: the alternative was built and measured, and the surprises below record why it was withdrawn.
...
- `uv run --with` gives `sys.executable` a path under a temporary environment that uv removes when the caller exits (`~/.cache/uv/builds-v0/.tmp*/bin/python`). An interpreter that outlives its launcher cannot start the Patchright bridge from that path; the live acceptance test failed with `No such file or directory` before the worker-command handoff was added. Executing already-running cells is unaffected because the interpreter is loaded before the directory disappears. Superseded: the launcher owns a persistent environment now, so no handoff is needed and no uv process is kept alive per session.
- `uv run` never executes the command in place: it forks the child and stays as the parent, which for a session means one uv process alive for the whole idle timeout. Measured here: about 54 MiB PSS per parent, of which 39 MiB is private anonymous memory that scales with the number of sessions, flat over time - a retained high-water mark from resolving and installing, not a leak. The first invocation that has to materialise an environment peaks near 146 MiB before settling.

- The per-call environment is not a per-call copy of the runtime, which settles the write-amplification worry that argued for installing it once. One `uv run --with` call builds an environment of 21 small files (42 KB: activation scripts, `CACHEDIR.TAG`, interpreter symlinks) and resolves the packages from uv's immutable archive instead - `surf_agent` imported from `~/.cache/uv/archive-v0/<hash>/lib/python3.11/site-packages`, extracted once per dependency set and shared by every call. A thousand cells therefore write about 42 MB of small files and delete them again, against an SSD endurance budget in the hundreds of terabytes, and the 140 MB payload is written once either way. The archive itself is the real disk consumer (58 GB on this machine, from all uv usage) and uv prunes it with `uv cache prune`.
- The persistent-environment alternative was built, measured and withdrawn, and the reason to record it is that the measurements cut both ways. Installing the requirement once - uv owning a project under the cache directory, the launcher running its interpreter - removed the uv parent entirely (one process per session at 32 MiB instead of two at about 100 MiB), removed the handoff, and let a later cell import after the launcher had exited. What it cost was everything around the environment rather than the environment itself: a generated project, a lock file, a stamp, a `skill-path` record, a per-skill-copy key and its orphan cleanup, and an in-place update that a running session could see. Every one of those existed only to answer "is the installed revision the pinned one" for a directory we owned. `uv tool install` cannot serve here either: it requires console scripts, and this package deliberately has none. Given a preference for owning no state and for nothing accumulating, per-call `uv run --with` won - uv deletes its own environments, the leftover temporary directories measured 5.2 MB across 37 of them, and the durable cost is the uv parent that keeps a session's environment alive.
...
- [ ] Withdrawn: the launcher installs the runtime per call through `uv run --with` again, and `SURF_SESSION_WORKER_COMMAND` is back. The two bullets above describe the alternative that was built and measured; `skills/surf/scripts/run.py` and `tests/test_skill_launcher.py` match their state before it.
...
- A launcher and the runtime it drives are pinned as a pair, and testing them apart produces a convincing false failure: a reverted launcher (which announces the worker command through `SURF_SESSION_WORKER_COMMAND`) run against the previous revision's runtime, which ignores that variable, spawned the worker with `sys.executable` - a path inside the environment the exiting launcher then deleted. A later cell failed with `FileNotFoundError` on that interpreter, which looked like a flaw in the reinstated design and was only a stale pin. The pin has to move in the same change as the launcher, which is what the release rule already asks for.
