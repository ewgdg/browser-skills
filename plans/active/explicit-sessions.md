...
- Matching the worker by command line was a heuristic an honest process could trip: a python process that merely mentioned the socket path, the module name and the worker command was picked first and killed while the real worker kept running, which restored the original orphan in a new form. Ownership now comes from the socket itself (`/proc/net/unix` inode to `/proc/<pid>/fd`), which mentioning the path cannot fake; the weaker command-line scan is kept only where `/proc` does not exist, and a host that cannot establish an owner reports that instead of guessing.
- Two kernel details made that lookup fail before it worked, and both look like a broken fix rather than a hard problem: a socket file's own inode is not the inode a descriptor reports, and `/proc/net/unix` can carry several rows for one path with a zero-inode row first. Reading every candidate inode and matching any of them is what makes it reliable.
- A finished worker is a zombie until its creator reaps it, and `kill(pid, 0)` succeeds for a zombie. Counting that as alive made every `--kill-session` pay the full ten-second wait, and told the caller to resume a corpse with `kill -CONT` after a cell had killed its own worker; liveness now reaps and reads the process state, so a dead interpreter reads as gone.
- Silence has two causes that no probe can separate: a wedged interpreter, and a healthy one that is busy inside a cell holding the interpreter lock. The message names both and tells the caller to let its own cell finish first, because the recovery commands differ in cost - stopping a busy interpreter discards the bindings and the in-flight cell.
- Ownership needs a uniqueness rule, not just a better source. Two genuine listeners at one path - an older worker whose socket file was deleted, and a newer worker that bound the same name - are both in the table and neither row says which is current, and the command-line claim can be satisfied by a process that merely names the path. Both cases are now "not established": the recovery refuses and names the manual command, because signalling a pid from an ambiguous answer can kill an innocent process and report success while the wedged interpreter stays wedged.
- The Linux-only assumption survived longer than it should have. Ownership and liveness are separate facts with separate portability: `/proc` answers both, while a host without it has `lsof` for ownership (macOS ships it) and `ps -o state=` for the zombie case, which `kill(pid, 0)` cannot see. Only provable ownership is signalled now - the command-line match is a message, not a licence - and the platform survey is recorded in `docs/research/unix-socket-owner-without-proc.md`.
