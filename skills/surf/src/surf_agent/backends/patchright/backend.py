    ...
    def call_tool(self, name: str, args: dict[str, Any] | None = None) -> str:
        ...
        try:
            ...
            try:
                ...
                pass
            raise SurfAgentError(f"Patchright bridge tool {name} failed: {detail}") from exc
        except urllib.error.URLError as exc:
            raise AxiBridgeUnavailable(f"Patchright bridge call failed: {exc}") from exc
    ...
    def _ensure_running(self) -> None:
        ...
        while time.monotonic() < deadline:
            if self._health_ok():
                return
            time.sleep(0.25)
        raise SurfAgentError("Patchright bridge did not become healthy; install Patchright and run `uv run python -m patchright install chrome`")
...
class PatchrightBackend:
    ...
    @property
    def client(self) -> PatchrightBridgeClient:
        # Tests and callers may replace `agent.patchright_client` after construction.
        return getattr(self.agent, "patchright_client", self._client)
    ...
    def close_matching(self, pattern: str) -> int:
        raise SurfAgentError("close-matching is not supported by Patchright backend yet", exit_code=2)
    ...
    def capture_page_metadata(self) -> Any:
        fallback = AgentPage(stable_patchright_page_id(self.agent.state_file.stem), backend=PATCHRIGHT_BACKEND)
        try:
...
def stable_patchright_page_id(thread: str) -> int:
    value = 0
    for char in thread:
