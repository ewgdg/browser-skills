...
The launcher installs the runtime pinned by this skill once per revision, into a cache directory it reuses and prunes when nothing is using it. If it reports a missing or invalid pin, the skill is incorrectly installed; report that instead of working around it. For dependency failures, read [launcher setup](docs/launcher.md).
